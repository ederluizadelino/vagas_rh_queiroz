...
export default async function DashboardPage({ searchParams }) {
  ...
  const where = { ...(filters.lojaId ? { lojaId: filters.lojaId } : {}), ...(filters.tipoVagaId ? { tipoVagaId: filters.tipoVagaId } : {}) };
  const [lojas, tipos, vagas] = await Promise.all([
    prisma.loja.findMany({ orderBy: { nome: "asc" } }),
    prisma.tipoVaga.findMany({ orderBy: { nome: "asc" } }),
    prisma.vaga.findMany({ where, include: { loja: true, tipoVaga: true }, orderBy: { updatedAt: "desc" } }),
  ]);
  ...
  return (
    <AppShell>
      <Header title="Dashboard" subtitle="Cards, filtros e gráficos com todos os dados da planilha e novos lançamentos." />
      <DashboardFilters lojas={lojas} tipos={tipos} />
